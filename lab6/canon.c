...
// judege if a statement do nothing but delare a const
static bool isNop(T_stm x) 
{  return x->kind == T_EXP && x->u.EXP->kind == T_CONST;
 }

// if one of the statement isNop, just remove it.
...
// judge if the statement and the expression can be commuted
...
static T_stm reorder(expRefList rlist) {
   ...
   // move call to the top layer
   // rewrite CALL node to ESEQ(MOVE(Temp t, CALL(fun, args), Temp t))
...
static C_stmListList next(T_stmList prevstms, T_stmList stms, Temp_label done)
{
  ...
  // end the current basic blcok and create a new one
  ...
  // if it is a lebel, rewrite the tree with a jump node and redo next on it. 
...
static void trace(T_stmList list)
{
  ...
  // change traced block T_stmList to NULL, rewrite
