...
Temp_tempList Temp_Union_tempList(Temp_tempList l, Temp_tempList r){
  Temp_tempList result = l;
  while(r){
    if(!Temp_In_tempList(r->head, l))
      result = Temp_TempList(r->head, result);
    r = r->tail;
  }
  return result;
}
...
Temp_tempList Temp_Sub_tempList(Temp_tempList l, Temp_tempList r){
  ...
  while(l){
    if(!Temp_In_tempList(l->head, r))
      result = Temp_TempList(l->head, result);
    l = l->tail;
  }
  return result;
}
...
bool Temp_Equal_tempList(Temp_tempList l, Temp_tempList r){
  ...
  for(list = l; list != NULL; list = list->tail){
    if(!Temp_In_tempList(list->head, r))
      return FALSE;
  }
  for(list = r; list != NULL; list = list->tail){
    if(!Temp_In_tempList(list->head, l))
      return FALSE;
  }
  return TRUE;
}
...
Temp_tempList Temp_Replace_tempList(Temp_tempList list, Temp_temp old, Temp_temp new){
  if(list != NULL){
    if(list->head == old)
      //return Temp_TempList(new, list->tail);
      return Temp_TempList(new, Temp_Replace_tempList(list->tail, old, new));
    else
      return Temp_TempList(list->head, Temp_Replace_tempList(list->tail, old, new));
  }else{
    return NULL;
  }
}
