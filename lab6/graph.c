...
int G_graph_nodecount(G_graph g){
  return g->nodecount;
}
...
G_nodeList G_Union_nodeList(G_nodeList l, G_nodeList r){
  G_nodeList result = l;
  while(r){
    if(!G_In_nodeList(r->head, l))
      result = G_NodeList(r->head, result);
    r = r->tail;
  }
  return result;
}

G_nodeList G_Sub_nodeList(G_nodeList l , G_nodeList r){
  G_nodeList result = NULL;
  while(l){
    if(!G_In_nodeList(l->head, r))
      result = G_NodeList(l->head, result);
    l = l->tail;
  }
  return result;
}

G_nodeList G_Intersect_nodeList(G_nodeList l, G_nodeList r){
  G_nodeList result = NULL;
  while(l){
    if(G_In_nodeList(l->head, r))
      result = G_NodeList(l->head, result);
...
bool G_In_nodeList(G_node node, G_nodeList list){
  while(list){
    if(node == list->head)
      return TRUE;
    list = list->tail;
